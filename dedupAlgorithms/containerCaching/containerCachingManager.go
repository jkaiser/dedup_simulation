...
// Kind of father struct which for creating and managing ContainerCaching instances.
type ContainerCachingManager struct {
	avgChunkSize          int
	...
	ccMap                     map[string]*ContainerCaching
	...
	TotalStatsList []ContainerCachingStatistics            // holds the summarized statistics for each gen
	StatsList      []map[string]ContainerCachingStatistics // holds the statistics of all _used_ ccs after each gen.
}

func NewContainerCachingManager(config ContainerCachingConfig, avgChunkSize int, clientServers string, outFilename string, ioTraceFile string) *ContainerCachingManager {
	cm := new(ContainerCachingManager)
	cm.config = config
	...
	cm.ccMap = make(map[string]*ContainerCaching)
	cm.traceReaderMap = make(map[string]algocommon.TraceDataReader)
	return cm
}

func (cm *ContainerCachingManager) Init() bool {
...
// Creates a new ContainerCaching instance. All instances creates by this function share the
// same ChunkIndex and BlockIndex.
func (cm *ContainerCachingManager) CreateContainerCaching() *ContainerCaching {
	cc := NewContainerCaching(cm.config,
		cm.avgChunkSize,
		cm.containerStorage,
		cm.containerCache,
...
func (cm *ContainerCachingManager) collectStatistic(ccs map[string]*ContainerCaching) (ContainerCachingStatistics, map[string]ContainerCachingStatistics) {

	stats := make(map[string]ContainerCachingStatistics)
	for host, cc := range ccs {
		stats[host] = cc.StatsList[len(cc.StatsList)-1]
	}

	var totalStats ContainerCachingStatistics
...
func (cm *ContainerCachingManager) RunGeneration(genNum int, fileInfoList []algocommon.TraceFileInfo) {
	...
	usedCCs := make(map[string]*ContainerCaching, 0)
	...
	for _, fileInfo := range fileInfoList {
		...
		if _, ok := cm.ccMap[hostname]; !ok {
			cm.ccMap[hostname] = cm.CreateContainerCaching()
		}
		...
		go func(gen int, ccHandler *ContainerCaching, tracePath string) {
			defer wg.Done()
...
func (cm *ContainerCachingManager) Quit() {
